...
const DesignRoomScreen = () => {
  ...
  const [backgroundColor, setBackgroundColor] = useState<string>("#fff"); // Default white background color
  ...
  const validateOutfit = useCallback((items: ClothingItem[]) => {
    ...
    if (hasSkirt && hasPants) {
      setOutfitWarning(
        "You have both a skirt and pants selected. Consider choosing one."
      );
    } else {
      setOutfitWarning("");
    }
  ...
  const handleUpdatePosition = useCallback(
    (id: number, x: number, y: number) => {
      setClothes((prevClothes) =>
        prevClothes.map((item) => (item.id === id ? { ...item, x, y } : item))
      );
    },
    []
  );
  ...
  useEffect(() => {
    ...
    const initialClothes = selectedItems.map((item, idx) => {
      ...
      if (item?.type === "shirt" || item?.type === "skirt") {
        ...
        yPosition = shirtItem
          ? draggableAreaTop + draggableAreaHeight * 0.45
          : draggableAreaTop + draggableAreaHeight * 0.3;
      } else if (item?.type === "shoes") {
        yPosition = pantsItem
          ? draggableAreaTop + draggableAreaHeight * 0.7
          : draggableAreaTop + draggableAreaHeight * 0.5;
      } else {
        yPosition = draggableAreaTop + draggableAreaHeight * 0.4;
      }
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "black" }}>
      ...
      <View style={{ flex: 1, marginHorizontal: 16, marginBottom: 120 }}>
        ...
        >
          ...
          {backgroundColor === "#fff" && (
            ...
            >
              {Array.from({ length: Math.floor((height * 0.75) / 25) }).map(
                (_, rowIndex) => (
                  <View key={rowIndex} style={{ flexDirection: "row" }}>
                    {Array.from({ length: Math.floor((width - 32) / 25) }).map(
                      (_, colIndex) => (
                        <View
      ...
      >
        ...
        >
          <View
            style={{
              ...
              backgroundColor:
                backgroundColors[
                  (backgroundColors.indexOf(backgroundColor) + 1) %
        ...
        >
          <Text style={{ color: "black", fontSize: 32, fontWeight: "400" }}>
            +
          </Text>
        </TouchableOpacity>
